...
The letters h-o-r-s-e do not inherently imply a hoofed mammal that run fast. In German, the same animal is “Pferd.” In Chinese, it is “馬.” The spellings are completely different, yet they refer to the same real-world creature — they are just *symbols* that represent the same concept.

Artificial neural networks, the core components of AI models, operate on numbers but not symbols. So before a model can process language, we must convert text into numbers.
...
First, we must break the text into smaller units called **tokens**. This is called **tokenization**.

```
Input:      "the ring fell"
Tokens:     ["the", "ring", "fell"]
               ↓      ↓       ↓
Token IDs:  [ 101,   205,    310]
```

Each token maps to a unique integer ID via a static lookup table (the *vocabulary*).
...
Think of the embedding matrix as a **dictionary**, but instead of definitions, each entry contains a list of numbers:
...
- $d_{model}$ = how many numbers describe each token

Each row corresponds to one token in the vocabulary. To "look up" a token, we simply retrieve its row.

#### What Do These Numbers Represent?

Each number in a token's vector captures some **feature** of that token's identity. You can think of it like this: just as a person might be described along dimensions like *height*, *age*, *introversion*, a token is described along learned dimensions.

The crucial distinction:

| Concept | What It Captures |
|---------|------------------|
| **Identity** | *What* this token is, in isolation—its inherent properties as a vocabulary item |
| **Meaning** | *What* this token means *here*—which requires context |

The embedding matrix only gives us **identity**. The vector for "ring" encodes properties like:
...
But this vector doesn't tell us whether "ring" means jewelry or sound in any particular sentence. That's **meaning**, and meaning requires **context**, which we don't have yet.

#### Are Features Human-Interpretable?
...
Each row is a token's **identity vector**—a numerical fingerprint that says *what* it is, but not yet *what it means here*.

We'll continue using $d_{model} = 4$ in the next chapter to keep examples consistent.
...
Now, the vector for "ring" depends on **what** it is and **where** it is.
